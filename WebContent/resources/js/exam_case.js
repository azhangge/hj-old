...
function CountDown(lockTime, passed)
{
    if (maxtime >= 0)
    {
        ...
        if ((passtime + passed) > lockTime * 60 && (!altered)) {
            altered = true;
...
/**
 * 本方法用于单题模式下提交试卷前验证是否仍有未做试题
 * 实现原理是 检查导航条中的按钮是否有“未做”的显示状态
 * @returns {Number|indx} 返回未做试题的序号
 */
function checkAllQuestionForSingleMode() {
    var result = -1;
    jQuery("span[id^='dial-']").each(function () {
        var classx = jQuery(this).attr('class');
        var ind = jQuery.trim(jQuery(this).text());
        try {
            if (classx.indexOf('dialB') !== -1) {
...
/**
 * 本方法用于多题模式下提交试卷前验证是否仍有未做试题
 * 实现原理是 检查导航条中的按钮是否有“未做”的显示状态
 * @returns {Number|id} 返回未做试题的ID
 */
function checkAllQuestionForMultiMode() {
    var result = '';
    jQuery("span[id^='dial-']").each(function () {
        var classx = jQuery(this).attr('class');
