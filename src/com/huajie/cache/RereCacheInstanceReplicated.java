...
import com.huajie.cache.topic.CacheMsgUtil;
import com.huajie.util.SpringHelper;

/**
 * 冗余缓存实例类，每个应用里边都重置缓存，相比本地缓存增加缓存消息广播功能
 * 当其中一台服务器的缓存被人为删除（非过期逐出）、修改、销毁时，所有缓存节点应该同步更新
 *
 * @author huajie.com
 */
public class RereCacheInstanceReplicated<T> implements IRereCacheInstance {
     ...
     * @param name
     */
    public RereCacheInstanceReplicated(String name) {
        this.name = name;
    ...
    public void add(String id, Object target) {
        this.put(id, target);
        CacheMsgUtil.publishAdd(name, id, target);
    }
    
    
    public void addWithoutMsg(String id, Object target) {
        this.put(id, target);
    }

    /**
     * 修改一个缓存条目中的对象，若存在
     * 每一个广播缓存消息的方法，都应该对应有一个不广播的版本，以在监听到其它节点消息时，本地更新后不再广播
     *
     * @param id
     * @param target
     */
    ...
    public void update(String id, Object target) {
        ...
        if (item != null) {
            ...
            RereCacheIndexUtil.checkIndexWithPut(this, target);
            CacheMsgUtil.publishUpdate(name, id, target);
        }
    }
    
    public void updateWithoutMsg(String id, Object target) {
        RereCacheItem item = this.get(id);
        if (item != null) {
            item.setObject(target);
            item.setVisitTime(new Date());
    ...
    public void remove(String id) {
        ...
        items.remove(id);
        CacheMsgUtil.publishDelete(name, id);
    }
    
    public void removeWithoutMsg(String id) {
        RereCacheIndexUtil.checkIndexWithDelete(this, id);//删除条目时应首先更新索引
        items.remove(id);
    }
    ...
    public void removeAll() {
        ...
        CacheMsgUtil.publishDestroy(name);
    }
    
    public void removeAllWithoutMsg() {
        this.indexes.clear();//清除索引
        this.items.clear();//清除数据条目
    }
    ...
    public void batchRemove(List ids) {
        if (ids != null) {
            for (Object idd : ids) {
                this.removeWithoutMsg(idd.toString());
            }
            CacheMsgUtil.publishBatchDelete(name, ids);
        }
    }
    
    public void batchRemoveWithoutMsg(List ids) {
        if (ids != null) {
            for (Object idd : ids) {
                this.removeWithoutMsg(idd.toString());
            }
    ...
    public static void main(String args[]) {
        RereCacheInstanceReplicated ci = new RereCacheInstanceReplicated("temp");
        ci.put("test", "111");
    }
