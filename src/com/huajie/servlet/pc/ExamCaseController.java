...
@WebServlet(urlPatterns = {"/servlet/pc/ExamCaseController"})
public class ExamCaseController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (case_id != null) {//恢复老考试
                ...
                String exam_type = examCase.isIfSimulate()?"1":"0";
                if (examCase != null) {
                    Examination exam = examCase.getExamination();
                    if (exam == null) {
                        return;
                    } else {
                        String mode = exam.getDisplayMode();
                        String paperType = exam.getPaperType();
                        if ("multiple".equals(mode)) {
                            url = "ExamCaseGeneral.jspx?case_id=" + case_id+"&exam_type="+exam_type;
                        } else {
                            url = "ExamCaseGeneralStep.jspx?case_id=" + case_id+"&exam_type="+exam_type;
                        }
                    }
                } else {
                    //若examCase==null
                ...
                Examination exam = this.examDAO.findExamination(exam_id);
                if (exam == null) {
                    return;
                } else {
                    String mode = exam.getDisplayMode();
                    String paperType = exam.getPaperType();
                    if ("multiple".equals(mode)) {
                            url = "ExamCaseGeneral.jspx?exam_id=" + exam_id+"&exam_type="+exam_type;
                    } else {
                            url = "ExamCaseGeneralStep.jspx?exam_id=" + exam_id+"&exam_type="+exam_type;
                    }
                }
            }

            String path = request.getContextPath() + "/talk/" + url;
