...
@WebFilter(urlPatterns = {"/talk/*", "/servlet/pc/*"})
public class Authentication2
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
                ...
                if (login) {//在登录状态跳转才有意义
                    try {
                        if (PartialRedirectHelper.isAjax(r)) {//AJAX跳转
                            ...
                            resp.getWriter().print(PartialRedirectHelper.xmlPartialRedirectToPage(r, r.getContextPath() +"/KickedOut.jspx"));
                            resp.flushBuffer();
                        } else {//非AJAX跳转
                            MyLogger.println("Repeated login, non-partial redirect.");
                            resp.sendRedirect(r.getContextPath() + "/KickedOut.jspx");
        ...
        if (!ifpasscheck) {
            try {
                if (!filtered) {
                    resp.sendRedirect(r.getContextPath() + "/CopyrightError.jspx");
                    chain.doFilter(request, response);
                    filtered = true;
                }
            } catch (Exception e) {
