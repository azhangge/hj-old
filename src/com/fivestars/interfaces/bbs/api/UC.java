 ...
 * 可以根据业务需要添加相应的执行代码
 *
 * 作者：梁平 创建时间：2009-2-20
 */
@WebServlet(urlPatterns = {"/api/uc.php"})
public class UC extends HttpServlet {
    ...
    /**
     * 执行具体的Action 所有服务器发出的参数均可通过$get来获得。 注意： request本身是不能得到参数值的。
     *
     * @param request
     * @param response
     * @return 操作状态或操作结果
    ...
    private String doAnswer(HttpServletRequest request, HttpServletResponse response) {
        ...
        if ($action.equals("test")) {
            ...
            //加入本系统能识别的cookies
            Cookie ck = new Cookie("rereexam.username", userName);
            ck.setMaxAge($cookietime);
            response.addCookie(ck);
            //登录考试系统
            MyLogger.println("UCENTER登入考试系统，用户名：" + userName);
            ComplexBbsUserService cbus = SpringHelper.getSpringBean("ComplexBbsUserService");
            cbus.loginFromOutside(request, userName, null, null, null);

            IUserSessionStateService iussService = SpringHelper.getSpringBean("UserSessionStateService");
            IBbsUserDAO userDAO = SpringHelper.getSpringBean("UserSessionStateService");
            BbsUser bu = userDAO.findBbsUserByUrn(userName);
            if (bu != null) {
            ...
            //考试系统退出登录
            MyLogger.println("UCENTER退出考试系统。");
            ClientSession cs = JsfHelper.getBean("clientSession");
            String result = cs.exit();
