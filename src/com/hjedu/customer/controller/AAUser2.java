...
@ManagedBean
@ViewScoped
public class AAUser2 implements Serializable {
...
//      String urn = this.user.getUsername().trim(); 
    ...
    public String onChangePhone() {
    	...
    	if (phone != null && !"".equals(phone)) {
    		if(Validator.isMobile(phone)){//是合格的手机号
				...
				AdminInfo ai = this.adminDAO.findAdminByPhone(truePhone);
				BbsUser bu = this.bbsUserDAO.findBbsUserByPhone(truePhone);
	            if (ai != null || bu != null) {
	                this.phoneOk = false;
    ...
    public List<ExamModuleModel> fetchModules() {
        ...
        List<ExamModuleModel> exxs = new ArrayList();
    ...
    public List<Examination> fetchExams() {
        List<Examination> exxs = new ArrayList();
        if (this.exams != null) {
    ...
    public List<LessonType> fetchLessonTypes() {
        List<LessonType> exxs = new ArrayList();
