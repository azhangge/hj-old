...
public class ExaminationService implements Serializable {
    ...
    public IRereCacheInstance getInstance() {
        if (ins == null && !StringUtils.isEmpty(CookieUtils.getBusinessId(JsfHelper.getRequest()))) {
    ...
    public List<Examination> findAllExamination() {
        IRereCacheInstance<List<Examination>> ci = getInstance();
        List<Examination> exams = ci.fetchObject(CookieUtils.getBusinessId(JsfHelper.getRequest())+"_exams");
        if (exams == null) {
            exams = examDAO.findAllExamination(CookieUtils.getBusinessId(JsfHelper.getRequest()));
