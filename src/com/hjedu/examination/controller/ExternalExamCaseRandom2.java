...
@ManagedBean
@ViewScoped
public class ExternalExamCaseRandom2 implements Serializable {
...
//            exec.execute(ecs);
            ...
            for (ExamCaseItemAdapter adapter : adapters) {
                if (adapter.getQtype().equals("choice")) {
                    ...
                    for (ExamCaseItemChoice ei : adapter.getCaseItem().getChoiceItems()) {
                        ...
                        ExamCaseItemAdapter ad = new ExamCaseItemAdapter();
                        ad.setChoiceItem(ei);
                        ad.setItem(ei);
                        ad.setVirtualPart(part);
                        ad.setQtype("case");
                        this.adapterMap.put(total, adapter);
                    }
    ...
    public boolean checkCurrent() {
        ...
        if (curentIfRight) {
            //this.nextAdapter();
            examCase.setProgress(this.currentIndex + 1);
            this.saveExamCase("auto");
        }
        System.out.println(this.curentIfRight);
        return this.curentIfRight;
    }
    ...
    public void submitExamCase() {
        ...
        this.submited = true;
        String result = "ExternalResult?faces-redirect=true";
       ...
       // bu.setScore(bu.getScore() + examCase.getBbsScore());
        ...
        em.setDateStr(dateStr);
        em.setUrn(bu.getUsername());
        em.setIfPass(true);
        em.setIp(JsfHelper.getRequest().getRemoteAddr());
        em.setExamId(this.examCase.getExamination().getId());
        new Thread(new ExamMessageSender(em)).start();//向调试中心发送信息
        //new Thread(new ExamMessageSender2(em)).start();//向业务系统发送信息
