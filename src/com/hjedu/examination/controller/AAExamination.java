...
@ManagedBean
@ViewScoped
public class AAExamination implements Serializable {
    
    ILogService logger = SpringHelper.getSpringBean("LogService");
    ExaminationService examDAO = SpringHelper.getSpringBean("ExaminationService");
    UserAppService userAppService = SpringHelper.getSpringBean("UserAppService");
    ...
    public void loadLabels() {
        ...
        if (lstr != null) {
            for (ExamLabelType lt : labelTypes) {
                List<ExamLabel> ls = lt.getExamLabels();
                for (ExamLabel e : ls) {
    ...
    private boolean testIfSelected(DictionaryModel dd) {
        if (this.exam!=null&&this.exam.getGroupStr()!=null&&this.exam.getGroupStr().trim().contains(dd.getId())) {
            dd.setSelected(true);
            return true;
        } else {
    ...
    private void initRoot(DictionaryModel dd, TreeNode node){
        List<DictionaryModel> ls = dicDAO.findAllSonDictionaryModel(dd.getId());
        Collections.sort(ls);
        dd.setSons(ls);
        Set<String> set = null;
        if(flag){
        	if(this.exam.getUserStr()!=null){
        		String[] str = this.exam.getUserStr().split(";");
        		if(str!=null){
    ...
    public void test(DictionaryModel dd, TreeNode node) {
        ...
        ks = dicDAO.cloneList(ks);
        for (DictionaryModel ddm : ks) {
            if (this.testIfSelected(ddm)) {
                k = true;
                ddm.setSelected(true);
                break;
            } else {
                ddm.setSelected(false);
            }
        }
        if (k) {
...
//获取结束
    ...
    private void setPaper() {
        if (exam.getRandomPaper() != null) {
            this.randomId = exam.getRandomPaper().getId();
        }
        if (exam.getRandom2Paper() != null) {
            this.random2Id = exam.getRandom2Paper().getId();
        }
        if (exam.getManualPaper() != null) {
            this.manualId = exam.getManualPaper().getId();
        }
    }
    ...
    private void resetPaper() {
        if (!"-1".equals(this.randomId)) {
            this.exam.setRandomPaper(this.randomDAO.findExamPaperRandom(randomId));
        }
        if (!"-1".equals(this.manualId)) {
            this.exam.setManualPaper(this.manualDAO.findExamPaperManual(manualId));
        }
        if (!"-1".equals(this.random2Id)) {
            this.exam.setRandom2Paper(this.random2DAO.findExamPaperRandom2(random2Id));
        }
	...
	public String done() {
		...
		save();
		return "ListExamination?faces-redirect=true";
	}
	...
	private boolean check() {
		...
		String type = exam.getPaperType();
		if(type.equals("random")){//简单随机试卷
			ExamPaperRandom paper = exam.getRandomPaper();
			if ("-1".equals(this.randomId)) {
				fm.setSummary("请选择简单随机试卷！");
				bool = false;
	        }
		}else if(type.equals("random2")){//随机试卷
			if ("-1".equals(this.random2Id)) {
				fm.setSummary("请选择随机试卷！");
				bool = false;
	        }
		}else{//人工试卷
			if ("-1".equals(this.manualId)) {
				fm.setSummary("请选择人工试卷！");
				bool = false;
	        }
		}
		return bool;
...
//		this.checkDepartment2();
		...
		if (flag) {
			...
			ExamPaperPool.refreshExamination(exam.getId());// 更新试卷池中的考试
			this.logger.log("修改了考试：" + exam.getName());
		} else {
			this.logger.log("添加了考试：" + exam.getName());
			this.exam.setBusinessId(CookieUtils.getBusinessId(JsfHelper.getRequest()));
			this.examDAO.addExamination(exam);
			ExamPaperPool.addExamination(exam);// 加入试卷池中的考试
		}
		AdminSessionBean asb = JsfHelper.getBean("adminSessionBean");
		asb.refreshAdmin();
		// createExaminationJsonFile(exam);
	}
    ...
    private void setUserStr() {
    	...
    	for(TreeNode node : selectedNodes){
    		if(node.getData() instanceof BbsUser){
    			...
    			if(!userIds.toString().contains(user.getId())){
    				userIds.append(user.getId());
    				userIds.append(";");
    			}
    		}
    	}
    	this.exam.setUserStr(userIds.toString());
	}
...
//    	save();
    	return "AAExamination2?id="+exam.getId()+"&faces-redirect=true";
    }
    
    public String done3() {
//    	save();
    	return "AAExamination?id="+exam.getId()+"&faces-redirect=true";
    }
