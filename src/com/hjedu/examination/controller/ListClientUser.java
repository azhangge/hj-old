...
@ManagedBean
@ViewScoped
public class ListClientUser implements Serializable {
	private static final Logger log4j = Logger.getLogger(ListClientUser.class);
    ...
    public void init() {
    	try {
	    	...
	    	this.casusList = this.cud.findAllBbsUser(businessId);
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    }
    ...
    public void batDeleteUser() {
        for (BbsUser bu : this.casusList) {
            if (bu.isSelected()) {
            	...
            	this.cud.deleteBbsUser(buid);
            	this.complexUserService.deleteBbsUserSafely(buid);
            	this.logger.log("删除了用户：" + bu.getId());
            }
        }
        synchronizeDB();
    }
...
//    		}
    	...
    	JSONObject jsonsyncVerify=Conn.syncVerifySubUser(uCenter,sysId, subtelmap1);
		...
		if(!subusermap.isEmpty()){
			JSONObject jsonsyncAdd=Conn.syncAddSubUser(uCenter,sysId,  subusermap);
    	...
    	JSONObject jsonsyncEid=Conn.syncEid(uCenter, sysId, subtelrnoeidmap);
...
//        }
    	...
    	JSONObject jsonSync=Conn.syncSubUser(uCenter,sysId, subtelmap2);
    ...
    public void someAbleUser2(String id) {
        ...
        if (cu.getEnabled()) {
        	//禁用用户
        	JSONObject disableSubUser=Conn.disableSubUserByTel(uCenter, sysId, cu.getTel());
        	String disableStatus=disableSubUser.getString("status");
        	...
        	//禁用用户
        	JSONObject enableSubUser=Conn.enableSubUserByTel(uCenter, sysId, cu.getTel());
        	String enableStatus=enableSubUser.getString("status");
