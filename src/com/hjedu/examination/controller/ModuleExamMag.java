...
@ManagedBean
@ViewScoped
public class ModuleExamMag implements Serializable {
    ...
    @PostConstruct
    public void init() {
        this.loadStructure(this.bussinessId);
        this.loadAdmins();
        this.loadStructure3();
    }
    ...
    public void loadStructure(String businessId) {
        this.root = new DefaultTreeNode();
//        ExamModuleModel dm = dicDAO.findExamModuleModel(this.businessId);
...
//获取结束
    ...
    public String delCases(String mid) {
        ExamModuleModel emm = this.dicDAO.findExamModuleModel(mid);
        ModuleExamination2 exam = emm.getExam();
        if (exam != null) {
            IModuleExamCaseDAO caseDAO = SpringHelper.getSpringBean("ModuleExamCaseDAO");
            caseDAO.deleteModuleExamCaseByExam(exam.getId());
        }
        
        JsfHelper.info("本章节的练习成绩已经清空！");
        return null;
    }
