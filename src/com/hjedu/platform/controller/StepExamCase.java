...
@ManagedBean
@ViewScoped
public class StepExamCase implements Serializable {
    ...
    public void init() {
        ...
        if (case_id != null) {//恢复老考试
            ...
            Examination exam = this.examinationDAO.findExamination(id);
            if (exam == null) {
                return;
            }
            if (exam.getPaperType().equals("random")) {
                examCaseService = SpringHelper.getSpringBean("ExamCaseService");
            } else if (exam.getPaperType().equals("random2")) {
...
//            exec.execute(ecs);
    ...
    public String submitExamCase() {
        ...
        this.submited = true;
        String result = "StepExamCaseResult?faces-redirect=true";
        ...
        if (appBean.getSystemConfig().getAsyncSubmit()) {
            ...
            if (this.examCaseDAO.findExamCase(examCase.getId()) == null) {
